...
adjArray *copyAdjArray(adjArray *src) {
  adjArray *dest = (adjArray*) malloc(sizeof(adjArray));
  dest->n = src->n; dest->m = src->m;
  dest->cd = (unsigned int*) malloc((src->n+1)*sizeof(unsigned int));
  memcpy(dest->cd, src->cd, (src->n+1)*sizeof(unsigned int));
  dest->adj = (unsigned int*) malloc(src->cd[src->n]*sizeof(unsigned int));
  memcpy(dest->adj, src->adj, src->cd[src->n]*sizeof(unsigned int));
  return dest;
}
